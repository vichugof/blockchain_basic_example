...
// var confirmTransactionsClient = function (node_url, data, mine) {
var confirmTransactionsClient = function (nodes, node_idx, data, mine) {
    var headers = {
        'Access-Control-Allow-Origin': '*'
    };
    ...
    $.ajax({
        ...
        url: nodes[node_idx] + "/transactions/new",
        type: "POST",
        headers: headers,
        dataType: 'json',
        data: JSON.stringify(data),
        ...
        success: function (response) {
            if (mine) {
                // mineNodeClient(node_url);
                mineNodeClient(nodes, node_idx,);
            }
        },
        error: function (error) {
            console.log(error);
...
var mineNodeClient = function (nodes, node_idx) {
    $.ajax({
        // url: node_url + "/mine",
        url: nodes[node_idx] + "/mine",
        type: "GET",
        success: function (response) {
            console.log('MINE', node_idx);
            for (var idx = 0; idx < nodes.length; idx++) {
                if (idx != node_idx) {
                    var timerResolve = setTimeout(resolveNodeClient, (idx+1)*1000, nodes[idx]);
                }
            }
        },
        error: function (error) {
...
var resolveNodeClient = function (node_url) {
    $.ajax({
        url: node_url + "/nodes/resolve",
        type: "GET",
        success: function (response) {
...
var registerNodesClient = function (node_url, data) {
    console.log(node_url, data);
    $.ajax({
        url: node_url + "/nodes/register",
        type: "POST",
